                    ...
                    <div class="row">
                        <div class="col-lg-4 col-md-3">
                            <a href="{{ route('ticket.show', $item->ticket->id) }}">
                                <div class="trend-item2 rounded">
                                    <img src="{{ asset('storage/ticket/' . $item->ticket->photo) }}" class="img" alt="">
                                </div>
                            </a>
                        </div>
                        ...
                        <div class="col-lg-3 col-md-3">
                            ...
                            <div class="d-flex align-items-center justify-content-end mb-2 me-1">
                                <button class="btn btn-sm btn-outline-primary" onclick="decreaseQty({{ $item->ticket->id }})">-</button>
                                <input type="text" id="qty-{{ $item->ticket->id }}" class="form-control text-center mx-2"
                                    value="{{ $item->quantity ?? '1' }}" style="width: 60px;" readonly>
                                <button class="btn btn-sm btn-outline-primary" onclick="increaseQty({{ $item->ticket->id }})">+</button>
                            </div>
                            <input type="hidden" id="price-{{ $item->ticket->id }}" value="{{ $item->ticket->price }}">
                        </div>
                    </div>
                </div>
                @endforeach
                            ...
                            <td colspan="6">
                                ...
                                <ul>
                                    @foreach ($order->carts as $cart)
                                        <li>{{ $cart->ticket->name }} - {{ $cart->quantity }} x Rp. {{ number_format($cart->ticket->price, 0, ',', '.') }}</li>
                                    @endforeach
                                </ul>
                            </td>
                        </tr>
